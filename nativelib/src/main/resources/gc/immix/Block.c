...
void Block_Recycle(Allocator *allocator, BlockHeader *blockHeader) {
    // If the block is not marked and does not contains old objects,
    // it means that it's completely free
    if (!Block_IsMarked(blockHeader) && !Block_ContainsOld(blockHeader)) {
        Block_recycleUnmarkedBlock(allocator, blockHeader);
        allocator->freeBlockCount++;
        allocator->freeMemoryAfterCollection += BLOCK_TOTAL_SIZE;
    } else if (!Block_IsMarked(blockHeader) && Block_ContainsOld(blockHeader)) {
        if (Block_IsRecyclable(blockHeader)) {
...
void Block_RecycleOld(Allocator *allocator, BlockHeader *blockHeader) {
    ...
    // If the block is not marked, it does not contains old
    // object so it's completely free
    if (!Block_IsMarked(blockHeader)) {
        Block_recycleUnmarkedBlock(allocator, blockHeader);
        allocator->freeBlockCount++;
        allocator->freeMemoryAfterCollection += BLOCK_TOTAL_SIZE;
    } else {
        // Contains old object so we need to recycle it
        // and re-mark unmarked old object
        assert(Block_IsMarked(blockHeader));
        Block_Unmark(blockHeader);
